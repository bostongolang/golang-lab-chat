...
//
// This is the main chatroom data structure.
//
...
//
// ListenForMessages will listen for chatroom messages.
//
// It runs in a goroutine and listens in a loop on the following channels
//
//	- cr.joins <- join requests from incoming clients
//	- cr.incoming <- messages from incoming clients
//	- cr.disconnects <- disconnect requests from incoming clients
//
func (cr *ChatRoom) ListenForMessages() {
	go func() {
		for {
			select {
			case data := <-cr.incoming:
				cr.Broadcast(data)
			case user := <-cr.joins:
